...
> Выбросы, которые видны на графике обоснованы вычислительной погрешностью. Так как численная реализация решения требует больших мощностей ЭВМ и длительного времени, берется большое и достаточно грубое разбиение варьируемых параметров при определении голевых ситуаций. Однако, это не умаляет показательности графиков, ведь они подверждают главную зависимость параметров. (В данном случае параметр площади на графике после прохождения некоторого оптимального угла по вертикали, при увеличении этого параметра, начинает резко уменьшатсья)
...
При анализе графиков можем заметить, что при удалении абсолютного значения угла по горизонтали от 0го значения, площадь, соответствующая такому углу уменьшается.
